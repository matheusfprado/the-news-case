...
export function AppModal({ title, subtitle, onClose, children, footer }: AppModalProps) {
  const dialogRef = useAccessibleOverlay(onClose);
  ...
  return createPortal(
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 p-1.5" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
      <section ref={dialogRef} role="dialog" aria-modal="true" aria-labelledby="app-modal-title" className="flex h-[calc(100dvh-12px)] w-full max-w-[430px] flex-col overflow-hidden rounded-xl border border-white/[0.04] bg-[#0b0b11] shadow-xl">
        <header className="flex min-h-14 shrink-0 items-center justify-between border-b border-[#302b16] px-4"><div><h1 id="app-modal-title" className="text-sm font-bold text-white">{title}</h1>{subtitle && <p className="mt-0.5 text-[10px] text-zinc-500">{subtitle}</p>}</div><button type="button" onClick={onClose} aria-label="Fechar" className="flex h-11 w-11 items-center justify-center rounded-full bg-[#181821] text-white hover:bg-[#252532] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#F5C000]"><X className="h-5 w-5" /></button></header>
