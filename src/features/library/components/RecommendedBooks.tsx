...
export function RecommendedBooks({ onAdd }: { onAdd: (book: BookSearchResult) => void }) {
  ...
  return (
    <section aria-labelledby="recommended-title">
      <h2 id="recommended-title" className="text-xs font-bold text-zinc-400">Sugestões de leitura</h2>
      ...
      {!loading && books.length > 0 && <div className="mt-2 grid grid-cols-3 gap-2">{books.map((book) => <button key={book.id} type="button" onClick={() => onAdd(book)} aria-label={`Adicionar ${book.title}`} className="group relative min-w-0 overflow-hidden rounded-lg bg-[#181822] text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#F5C000]">{book.coverUrl ? <img src={book.coverUrl} alt={`Capa de ${book.title}`} loading="lazy" className="aspect-[.72] w-full object-cover" /> : <span className="flex aspect-[.72] items-center justify-center"><BookOpen className="h-5 w-5 text-zinc-700" /></span>}<span className="absolute right-1.5 top-1.5 flex h-6 w-6 items-center justify-center rounded-full bg-[#F5C000] text-black opacity-0 shadow transition-opacity group-hover:opacity-100 group-focus-visible:opacity-100"><Plus className="h-3.5 w-3.5" /></span><span className="block p-2"><strong className="block truncate text-[8px] text-white">{book.title}</strong><span className="block truncate text-[7px] text-zinc-600">{book.author}</span></span></button>)}</div>}
